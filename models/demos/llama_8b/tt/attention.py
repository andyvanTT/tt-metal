...
class Attention(LightweightModule):
    ...
    def forward_prefill(
        ...
        if batch_size > 1:
            ...
            # user_id is a list of the admitted requests' slot indices, but
            # k_fill/v_fill and the (packed) page table are indexed by request
            # position, so iterate rows positionally. The slot's KV blocks are
            # selected by the page-table contents, not the row index.
            ...
            for local_idx, slot_idx in enumerate(valid_slots):
                # Extract this request's K/V slice: [1, n_kv_heads, seq_len_per_user, head_dim]
                k_user = k_fill[local_idx : local_idx + 1, :, :, :]
                v_user = v_fill[local_idx : local_idx + 1, :, :, :]
                ...
                # Fill cache for this request with its packed page-table row
                ttnn.experimental.paged_fill_cache(keys_BKSD, k_user_sliced, fill_page_table, batch_idx=local_idx)
                ttnn.experimental.paged_fill_cache(values_BKSD, v_user_sliced, fill_page_table, batch_idx=local_idx)
