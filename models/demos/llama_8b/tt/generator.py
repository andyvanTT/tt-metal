...
class Generator(ModelCapabilitiesMixin, WarmupForwardMixin):
    ...
    def prefill_forward_text(
        ...
        **kwargs,
        ...
        for idx, user_id in enumerate(all_users):
            ...
            if use_batched_prefill:
                # Galaxy 70B approach: pack requests into rows 0..batch-1 of a
                # [padded_batch, prefill_seq_len] buffer; the request->slot
                # mapping is carried by empty_slots, not by row position, so
                # non-contiguous/partial slot sets work.
                ...
                for local_idx, slot in enumerate(empty_slots):
                    ...
                    prefill_ids[local_idx : local_idx + 1] = padded_tokens
                    padded_last_token_idx[local_idx] = last_token_idx[local_idx]
            ...
            if use_batched_prefill:
                ...
                if sampling_enabled:
                    ...
                    for local_idx, slot in enumerate(empty_slots):
                        ...
                        # seed/prompt state is slot-organized (32 rows), but
                        # prefill_ids is packed by request -- read local row
                        combined_prompt_tokens[slot, :plen] = prefill_ids[local_idx, :plen]
                    ...
                    for local_idx, slot in enumerate(empty_slots):
                        # rows are packed by request end-to-end (row j ==
                        # input row j == output row j)
                        output_tokens[local_idx] = tokens_host[local_idx]
                        if log_probs_host is not None:
                            output_log_probs[local_idx] = log_probs_host[local_idx]
                else:
                    if return_hidden_states:
                        # Embedding models: trace returns hidden states; extract last-token hidden per slot
                        slot_hidden_list = []
                        for local_idx, slot in enumerate(empty_slots):
                            user_hidden = logits[local_idx : local_idx + 1, :, :, :]
                            slot_hidden = self.model[model_id].process_hidden_states_after_prefill_trace(
                                user_hidden, last_token_idx[local_idx]
                            )
                            slot_hidden_list.append((local_idx, slot_hidden, last_token_idx[local_idx]))
                        ttnn.synchronize_device(self.model[model_id].mesh_device)
                        ...
                        for local_idx, slot in enumerate(empty_slots):
                            user_logits = logits[local_idx : local_idx + 1, :, :, :]
                            _logits = self.model[model_id].process_logits_after_prefill_trace(
                                user_logits, last_token_idx[local_idx]
                            )
                            ...
                            output_tensor[local_idx] = self.model[model_id].process_output_prefill(
                                _logits.cpu(), last_token_idx=(last_token_idx[local_idx] % 32)
                            )
                break
        ...
        **kwargs,
        ...
        if sampling_params is not None:
            ...
            for i in range(self.data_parallel):
                ...
                # Force-argmax never consumes seeds; skip the per-step host-to-
                # device seed copy. Besides being wasted work, the async write
                # races the traced decode replay and intermittently wedges the
                # command-queue completion tracking (host blocks forever on a
                # read while the device sits idle) on long continuous-batching
                # runs.
                if not (sampling_module.tt_sampling.force_argmax_sampling and not sampling_module._penalties_active):
    ...
    def _get_prefill_user_page_table(
        ...
        if use_batched_prefill:
            ...
            # page_table rows are packed per request (row i == request i, whose
            # slot is user_id[i]); the prefill kernel consumes rows
            # positionally, so keep the padded table packed the same way.
            for i, user in enumerate(user_id):
                padded_page_table[i, :] = page_table[i, :]
